...
use UniMethod\JsonapiMapper\Exception\JsonApiException;
...
class Deserializer
{
    ...
    public function handle(array $data, string $method, string $included = ''): object
    {
        ...
        $class = $objectConfig->getClass();
        $parseIncluded = $this->parseIncluded($included);

        $id = $data['data']['id'] ?? null;

        if ($id === null) {
            throw new JsonApiException('Please provide non nullable ID');
        }
        ...
        if ($method === Method::CREATE) {
            $object = new $class;
            $isNew = true;
        } elseif ($method === Method::UPDATE) {
            if (!$objectConfig->isAutoGeneratedId()) {
                $object = new $class;
            } else {
            ...
            if ($object === null) {
                throw new JsonApiException('No objects with this type and id');
            }
        } else {
            throw new ConfigurationException('You can handle objects only in CREATE and UPDATE methods');
        }

        if (!$objectConfig->isAutoGeneratedId()) {
            $this->setId($object, $objectConfig->getId(), $id);
        }
     ...
     * @throws ConfigurationException
     */
    protected function resolveObject(object $object, EntityConfig $objectConfig, array $data, bool $isNew, array $included): object {
    ...
    protected function getInnerObject(array $relationLink, array $links, array $included): object
    {
        ...
        $internalObject = $this->setAttributes(
            $this->getOuterObject($config->getClass(), $relationLink['id']),
            $config,
            $internalData
        );
    ...
    protected function getOuterObject(string $class, ?string $id): object
    {
        ...
        if ($config->isAutoGeneratedId() === false) {
            $object = new $class;
            if ($id !== null) {
                $this->setId($object, $config->getId(), $id);
            }
            return $object;
        }
    ...
    protected function setValue(object $object, string $prop, $value): object
    {
        if ($this->isMethod($prop)) {
            ...
            try {
                $reflectionAttr = new ReflectionProperty($object, $prop);

                if ($reflectionAttr->isPublic()) {
                    $object->$prop = $value;
                } else {
                    $reflectionAttr->setAccessible(true);
                    $reflectionAttr->setValue($object, $value);
                }
    ...
    /**
     * @param object $object
     * @param AttributeConfig $id
     * @param $value
     * @return void
     */
    protected function setId(object $object, AttributeConfig $id, $value): void
    {
        $this->setValue($object, $id->setter, $value);
    }
}
